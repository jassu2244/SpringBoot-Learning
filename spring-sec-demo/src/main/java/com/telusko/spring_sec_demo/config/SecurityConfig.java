...
public class SecurityConfig {
    ...
    //here I am defining what security I need to my site rather than spring security default configs
    @Bean
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(customizer -> customizer.disable())
                .authorizeHttpRequests(request -> request
                        .requestMatchers("/register", "/login") // this is done so that we server does not check
                        //for credentials for these 2 urls
                        .permitAll()
